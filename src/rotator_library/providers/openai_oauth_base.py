...
class OpenAIOAuthBase:
        ...
        async with await self._get_lock(path):
            ...
            try:
                ...
                with open(path, "r") as f:
                    creds = json.load(f)
                if self.ENV_PREFIX == "CODEX":
                    creds = normalize_codex_credential_payload(creds, path)
    ...
    def _is_access_token_only_credential(self, creds: Dict[str, Any]) -> bool:
        """Return True for imported/session credentials that cannot refresh."""
        return bool(creds.get("access_token")) and not bool(
            creds.get("refresh_token") or creds.get("api_key")
        )
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    if self._is_access_token_only_credential(creds):
                        if self._is_token_truly_expired(creds):
                            self._unavailable_credentials[path] = time.time()
    ...
    async def initialize_token(
        ...
        try:
            ...
            elif not has_api_key and not has_access_token:
                reason = "access token and API key are missing"
            elif not has_api_key and self._is_token_expired(creds):
                if has_refresh_token:
                    reason = "token is expired"
                elif self._is_token_truly_expired(creds):
    ...
    async def get_auth_header(self, credential_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Imported ChatGPT Web/session exports often have only an access token.
            # They are still valid for API and quota calls until their real exp,
            # but there is no refresh token to exchange, so do not queue/attempt refresh.
            if self._is_access_token_only_credential(creds):
                if self._is_token_truly_expired(creds):
                    raise ValueError(
                        f"Access token expired for {Path(credential_path).name} and no refresh_token is available."
                    )
                return {"Authorization": f"Bearer {creds['access_token']}"}

            # Fall back to refreshable access token
        ...
        for cred_file in sorted(glob(pattern)):
            try:
                with open(cred_file, "r") as f:
                    creds = json.load(f)
                if self.ENV_PREFIX == "CODEX":
                    creds = normalize_codex_credential_payload(creds, cred_file)
    ...
    def build_env_lines(self, creds: Dict[str, Any], cred_number: int) -> List[str]:
        ...
        """
        if self.ENV_PREFIX == "CODEX":
            creds = normalize_codex_credential_payload(creds, f"{self.ENV_PREFIX} credential")
    ...
    def export_credential_to_env(
        ...
        try:
            ...
            with open(cred_path, "r") as f:
                creds = json.load(f)
            if self.ENV_PREFIX == "CODEX":
                creds = normalize_codex_credential_payload(creds, str(cred_path))
