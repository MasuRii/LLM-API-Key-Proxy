...
with _console.status("[dim]Loading LiteLLM library...", spinner="dots"):
    ...
    _original_raise_on_model_repetition = getattr(
        CustomStreamWrapper,
        "raise_on_model_repetition",
        None,
    )

    if callable(_original_raise_on_model_repetition):
...
async def streaming_response_wrapper(
    ...
    """
    collect_response_chunks = logger is not None
    response_chunks = [] if collect_response_chunks else None
    full_response = {}
    ...
    try:
        async for chunk_str in response_stream:
            if await request.is_disconnected():
                ...
                break
            yield chunk_str
            if collect_response_chunks and chunk_str.strip() and chunk_str.startswith("data:"):
        ...
        if collect_response_chunks and response_chunks:
            # --- Aggregation Logic ---
